    ...
    class CurrentState
    {
        ...
        public CurrentState()
        {
            ...
            try
            {
                    ...
                    foreach (Wlan.WlanBssEntry network in Helper.getCurrentNetworks()) {
                        if (network.linkQuality > FNWifiLocator.Properties.Settings.Default.min_signal_value)
                        {
        ...
        public override bool Equals(System.Object obj)
        {
            ...
            if ((System.Object)p == null)
            {
                ...
                foreach (var pair in p.network_list){
                    ...
                    if (this.network_list.ContainsKey(pair.Key)
                      && ssid_1.Equals(ssid_2) //  (Encoding.ASCII.GetChars(pair.Value.dot11Ssid.SSID, 0, (int)pair.Value.dot11Ssid.SSID.Length)).Equals(Encoding.ASCII.GetChars(this.network_list[pair.Key].dot11Ssid.SSID, 0, (int)this.network_list[pair.Key].dot11Ssid.SSID.Length))
                        && (pair.Value.linkQuality <= this.network_list[pair.Key].linkQuality + FNWifiLocator.Properties.Settings.Default.delta_signal_value) &&
                        (pair.Value.linkQuality >= this.network_list[pair.Key].linkQuality - FNWifiLocator.Properties.Settings.Default.delta_signal_value)
                       ){
                ...
                Log.trace("Uguali:\t" + e.ToString() + "\tdiverse:" + d.ToString() + "\tpercentuale:" + (100 * e / (d + e)).ToString() + " VS " + FNWifiLocator.Properties.Settings.Default.min_perc_same_place.ToString());
                if ((100 * e / (d + e)) > FNWifiLocator.Properties.Settings.Default.min_perc_same_place)
                {
                    return true;
                }else{
            ...
            for (int k = 0; k < 1; k++)
            {
                ...
                //datapds1Entities2 db = new datapds1Entities2();
                    ...
                    foreach (RightPlace rp in rightplaces.Values)
                    {
                        ...
                        if (max == rp.avg && max > FNWifiLocator.Properties.Settings.Default.min_perc_same_place)
                        {
                    ...
                    if (this.current_place != null)
                    {
                        ...
                        foreach (Wlan.WlanBssEntry network in networks)
                        {
                            ...
                            else
                            {
                                ...
                                pnv.variance = (short)FNWifiLocator.Properties.Settings.Default.delta_signal_value;
